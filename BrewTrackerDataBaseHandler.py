...
class HomeBrewTrackerDataBase():
    ...
    def create_new_gravity_reading(self, beer_id, gravity_reading_time, gravity_reading):
        print("create_new_gravity_reading")
        print(str(gravity_reading_time) + str(gravity_reading))
        conn = sqlite3.connect("brewtrackerdb.db")
        cursor = conn.cursor()
        sql = "INSERT INTO gravity_readings (id, gravity_reading_time, gravity_reading) VALUES ('"
        sql = sql + str(beer_id) + "', '" + str(gravity_reading_time) + "', '" + str(gravity_reading) + "')"
        cursor.execute(sql)
        conn.commit()
    ...
    def get_gravity_readings_for_beer(self, beer_id):
        ...
        cursor = conn.cursor()
        gravity_readings = []
        for reading in cursor.execute("SELECT * FROM gravity_readings WHERE id='" + str(beer_id) + "'"):
            gravity_readings.append(reading)
